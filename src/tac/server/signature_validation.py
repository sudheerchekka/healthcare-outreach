...
def build_http_signature_dependency(
    ...
    async def _validate_http_signature(request: Request) -> None:
        ...
        if not validate_twilio_webhook(request, auth_token, body):
            # TODO: re-enable strict validation — temporarily logging instead of rejecting
            # to diagnose ngrok URL/signature mismatch
            import logging as _log
            _log.getLogger("tac.server.signature_validation").warning(
                f"[sig] INVALID signature from {request.client.host if request.client else '?'} "
                f"url={request.url} — allowing through for debugging"
            )
